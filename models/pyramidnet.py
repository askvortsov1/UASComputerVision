"""
Adapted from https://github.com/dyhan0920/PyramidNet-PyTorch/blob/master/PyramidNet.py
and 
https://github.com/owruby/shake-drop_pytorch/tree/master/models
"""
...
__all__ = ['pyramidnet', 'shake_pyramidnet']
...
class PyramidNet(nn.Module):
    ...
    def __init__(self, dataset, depth, alpha, num_classes, bottleneck=False):
        ...
        if self.dataset.startswith('cifar') or self.dataset.startswith('svhn'):
            self.inplanes = 16
            if bottleneck == True:
                n = int((depth - 2) / 9)
                block = Bottleneck
...
def shake_pyramidnet(args):
    return ShakePyramidNet(label=args.n_classes, depth=args.depth, alpha=args.alpha)


def pyramidnet(args):
    return PyramidNet(dataset=args.dataset, depth=args.depth, alpha=args.alpha,
                      num_classes=args.n_classes, bottleneck=args.bottleneck)
